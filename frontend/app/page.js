    ...
    <div className="flex h-screen bg-background font-sans text-foreground overflow-hidden">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-panel border border-panel-border rounded-2xl p-8 max-w-sm w-full shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              <span className="w-10 h-10 rounded-full bg-red-950 border border-red-800 flex items-center justify-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">This will remove all security events from the dashboard. This action cannot be undone.</p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowClearConfirm(false)}
                className="flex-1 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-semibold rounded-lg transition-all border border-slate-700"
              >
                Cancel
              </button>
              <button
      ...
      <aside className="w-64 bg-panel border-r border-panel-border flex flex-col justify-between shrink-0">
        <div>
          ...
          <nav className="p-4 space-y-1">
            {[
              ...
              { id: "blocked", label: "Intercepted Attacks", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />, badge: threatsBlocked > 0 ? <span className="ml-auto px-2 py-0.5 text-xs font-mono font-bold rounded-full bg-red-950 text-red-400 border border-red-800">{threatsBlocked}</span> : null },
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${
                  activeTab === tab.id
                    ? tab.id === "blocked" ? "bg-red-600/10 text-red-400 border border-red-600/30"
        ...
        <div className="p-4 border-t border-panel-border bg-panel">
          ...
          <div className="space-y-2">
            <button onClick={() => triggerSimulation("safe")} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-emerald-400 bg-emerald-950/20 hover:bg-emerald-950/40 border border-emerald-900/60 rounded-md transition-all">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
              Inject Safe Traffic
            </button>
            <button onClick={() => triggerSimulation("threat")} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-red-400 bg-red-950/20 hover:bg-red-950/40 border border-red-900/60 rounded-md transition-all">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-ping"></span>
              Inject Threat Attack
            </button>
          </div>
          <div className="mt-4 pt-4 border-t border-panel-border text-[10px] text-slate-500 font-mono text-center">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 border-b border-panel-border bg-panel px-8 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold rounded-lg transition-all border border-slate-700"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <span className="text-xs font-mono text-slate-500">CONNECTION:</span>
            {wsStatus === "connected" && (
        ...
        {activeTab !== "rules" && (
          <section className="p-8 pb-4 grid grid-cols-4 gap-6 shrink-0 bg-background">
            {[
              ...
              <div key={stat.label} className="bg-panel p-5 rounded-xl border border-panel-border hover:border-slate-300 dark:hover:border-slate-700/80 transition-all shadow-md group">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{stat.label}</span>
                  <span className={`text-${stat.color}-500 p-1.5 rounded-lg bg-${stat.color}-950/30 border border-${stat.color}-950`}>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>{stat.icon}</svg>
                  </span>
                </div>
                <h3 className={`text-3xl font-bold font-mono ${stat.color === "blue" ? "text-slate-100 group-hover:text-blue-400" : stat.color === "red" ? "text-red-500" : stat.color === "emerald" ? "text-emerald-500" : "text-purple-400 group-hover:text-purple-300"} transition-colors`}>
                  {stat.value}
                </h3>
                <p className="text-[11px] text-slate-500 mt-2">{stat.sub}</p>
              </div>
            ))}
          </section>
        )}
        ...
        {showAlert && latestThreat && (
          <div className="mx-8 mt-2 p-4 bg-red-950/30 border border-red-800/80 text-red-200 rounded-xl flex items-center justify-between shadow-2xl shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-red-300 mt-0.5">
                  Jailbreak payload intercepted from <strong className="font-mono text-white">{latestThreat.ip}</strong>. Reason: <strong>{latestThreat.violatedRule}</strong>
                </p>
              </div>
            </div>
            <button onClick={() => setShowAlert(false)} className="text-red-400 hover:text-red-200 px-3 py-1 text-xs border border-red-800 hover:border-red-600 rounded bg-red-950/50 transition-colors">
              Acknowledge
            </button>
          </div>
        )}
        ...
        <section className="flex-1 p-8 min-h-0 flex gap-6 overflow-hidden">
          ...
          {activeTab === "rules" ? (
            <div className="flex-1 bg-panel border border-panel-border rounded-xl p-8 overflow-y-auto shadow-lg">
              <div className="max-w-2xl">
                ...
                <div className="space-y-4">
                  {GUARDRAIL_RULES.map((rule) => (
                    <div key={rule.id} className="p-4 bg-background border border-panel-border rounded-lg flex gap-4 hover:border-slate-700 transition-colors">
                      <div className="w-6 h-6 rounded-md bg-blue-950 border border-blue-900 text-blue-400 flex items-center justify-center font-bold text-xs font-mono shrink-0">{rule.id}</div>
                      <div>
            ...
            <div className="flex-1 flex flex-col gap-4 min-w-0 overflow-hidden">
              ...
              {chartData.length > 0 && (
                <div className="bg-panel border border-panel-border rounded-xl p-4 shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-slate-400 font-mono uppercase">Risk Score Timeline (Last {chartData.length} Events)</span>
                      ...
                      return (
                        <div key={i} className="flex-1 flex flex-col items-center group relative">
                          <div
                            ...
                            className={`w-full rounded-sm transition-all ${isBlocked ? "bg-red-500 group-hover:bg-red-400" : "bg-emerald-500/60 group-hover:bg-emerald-400"}`}
                          ></div>
                          <div className="absolute bottom-full mb-1 hidden group-hover:block bg-slate-800 text-[9px] text-slate-200 px-1.5 py-0.5 rounded whitespace-nowrap z-10">
              ...
              <div className="flex-1 bg-panel border border-panel-border rounded-xl flex flex-col min-w-0 shadow-lg overflow-hidden">
                <div className="p-4 border-b border-panel-border flex justify-between items-center bg-panel rounded-t-xl shrink-0">
                  <span className="text-xs font-semibold text-slate-400 font-mono">FULL AUDIT LOG ({logs.length} total events)</span>
                  <button
                    onClick={downloadCSV}
                    disabled={logs.length === 0}
                    className="flex items-center gap-2 px-3 py-1.5 bg-blue-600/10 hover:bg-blue-600/20 disabled:opacity-40 disabled:cursor-not-allowed text-blue-400 text-xs font-semibold border border-blue-800 rounded-lg transition-all"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="flex-1 overflow-y-auto">
                  {logs.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center p-8 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-600 mt-1">🛡 System Active — Waiting for incoming prompts.</p>
                    </div>
                  ) : (
                    <table className="w-full text-left border-collapse table-fixed">
                      <thead className="sticky top-0 bg-panel border-b border-panel-border z-10">
                        <tr>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-32">Event ID</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-40">Timestamp</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-32">Source IP</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider">Query</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-24">Status</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-20">Risk</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-20">Confidence</th>
                          <th className="py-3 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-44">Violated Rule</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-panel-border">
                        {logs.map((log) => (
                          <tr key={log.id} onClick={() => setSelectedLog(log)} className="cursor-pointer hover:bg-slate-800/40 transition-colors">
                            <td className="py-3 px-4 font-mono text-[11px] text-slate-500">{log.id}</td>
                            <td className="py-3 px-4 font-mono text-[11px] text-slate-400">{isMounted ? new Date(log.timestamp).toLocaleString() : ""}</td>
                            <td className="py-3 px-4 font-mono text-[12px] text-slate-300 font-semibold">{log.ip}</td>
                            <td className="py-3 px-4 text-[12px] text-slate-200 overflow-hidden text-ellipsis whitespace-nowrap">{log.query}</td>
                            <td className="py-3 px-4">
                              {log.status === "Blocked" ? (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-red-950 text-red-400 border border-red-900">
                                  <span className="w-1.5 h-1.5 rounded-full bg-red-500"></span>Blocked
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-emerald-950 text-emerald-400 border border-emerald-900">
                                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>Safe
                                </span>
                              )}
                            </td>
                            <td className="py-3 px-4 font-mono text-[12px] font-bold">
                              <span className={parseFloat(log.riskScore) > 0.7 ? "text-red-400" : "text-emerald-400"}>{log.riskScore}</span>
                            </td>
                            <td className="py-3 px-4 font-mono text-[11px] text-slate-400">{log.confidence}</td>
                            <td className="py-3 px-4 font-mono text-[11px] text-slate-400 overflow-hidden text-ellipsis whitespace-nowrap">
                              {log.status === "Blocked" ? <span className="text-red-400">{log.violatedRule}</span> : <span className="text-slate-600">—</span>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            <div className="flex-1 bg-panel border border-panel-border rounded-xl flex flex-col min-w-0 shadow-lg">
              <div className="p-4 border-b border-panel-border flex justify-between items-center bg-panel rounded-t-xl shrink-0">
                <span className="text-xs font-semibold text-slate-400 font-mono">
                  {activeTab === "blocked" ? `INTERCEPTED ATTACKS (${displayLogs.length} events)` : `LIVE EVENT STREAM (${displayLogs.length} events)`}
                </span>
                <span className="text-[10px] text-slate-500 font-mono">Click any entry to open forensic inspector</span>
              </div>
              <div className="flex-1 overflow-y-auto">
                {displayLogs.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center p-8 text-center text-slate-500">
                    ...
                    <p className="text-xs text-slate-600 mt-1">Waiting for incoming prompts. Run the simulator or use the Live Demo to generate traffic.</p>
                  </div>
                ) : (
                  <table className="w-full text-left border-collapse table-fixed">
                    <thead className="sticky top-0 bg-panel border-b border-panel-border z-10">
                      <tr>
                        <th className="py-3.5 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-36">Timestamp</th>
                        <th className="py-3.5 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-32">Source IP</th>
                        <th className="py-3.5 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider">User Request Query</th>
                        <th className="py-3.5 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-24">Status</th>
                        <th className="py-3.5 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-16">Risk</th>
                        <th className="py-3.5 px-4 text-[11px] font-semibold text-slate-400 font-mono tracking-wider w-44">Guardrail Intercept</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-panel-border">
                      {displayLogs.map((log) => {
                        ...
                        return (
                          <tr
                            ...
                            className={`group cursor-pointer hover:bg-slate-800/40 transition-colors ${isSelected ? "bg-slate-800/80" : ""}`}
                          >
                            <td className="py-3.5 px-4 font-mono text-[11px] text-slate-400">{isMounted ? new Date(log.timestamp).toLocaleTimeString() : ""}</td>
                            <td className="py-3.5 px-4 font-mono text-[12px] text-slate-300 font-semibold overflow-hidden text-ellipsis whitespace-nowrap">{log.ip}</td>
                            <td className="py-3.5 px-4 text-[13px] text-slate-200 overflow-hidden text-ellipsis whitespace-nowrap font-medium pr-8">{log.query}</td>
                            <td className="py-3.5 px-4">
                              {log.status === "Blocked" ? (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-red-950 text-red-400 border border-red-900">
                            ...
                            <td className="py-3.5 px-4 font-mono text-[12px] font-bold">
                              <span className={parseFloat(log.riskScore) > 0.7 ? "text-red-400" : "text-emerald-400"}>{log.riskScore}</span>
                            </td>
                            <td className="py-3.5 px-4 font-mono text-[11px] font-semibold overflow-hidden text-ellipsis whitespace-nowrap">
                              {log.status === "Blocked" ? <span className="text-red-400">{log.violatedRule}</span> : <span className="text-slate-500">—</span>}
                            </td>
                          </tr>
                        );
                      })}
          ...
          {selectedLog && activeTab !== "rules" && (
            <div className="w-96 bg-panel border border-panel-border rounded-xl p-6 flex flex-col justify-between shadow-xl shrink-0 relative font-sans">
              <button onClick={() => setSelectedLog(null)} className="absolute top-4 right-4 text-slate-500 hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-800 transition-all">
              ...
              <div className="overflow-y-auto space-y-5 pr-1">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 font-mono mt-1">{isMounted ? new Date(selectedLog.timestamp).toLocaleString() : ""}</div>
                </div>
                    ...
                    <div>
                      ...
                      <div className="mt-1">
                        {selectedLog.status === "Blocked" ? (
                          <span className="inline-flex px-2 py-0.5 rounded bg-red-950 border border-red-950 text-red-400 font-mono text-xs font-bold uppercase">Blocked</span>
                        ) : (
                          <span className="inline-flex px-2 py-0.5 rounded bg-emerald-950 border border-emerald-950 text-emerald-400 font-mono text-xs font-bold uppercase">Allowed</span>
                        )}
                      </div>
                    </div>
                    <div>
                      <span className="text-xs text-slate-500 uppercase font-semibold">Risk Score</span>
                      <p className={`text-sm font-mono font-bold mt-1 ${parseFloat(selectedLog.riskScore) > 0.7 ? "text-red-400" : "text-emerald-400"}`}>{selectedLog.riskScore}</p>
                    </div>
                  </div>
                  <div>
                    <span className="text-xs text-slate-500 uppercase font-semibold">Confidence</span>
                    <p className="text-sm font-mono font-bold mt-1 text-slate-300">{selectedLog.confidence}</p>
                  </div>
                  <div>
                    <span className="text-xs text-slate-500 uppercase font-semibold">Intercept Rules Triggered</span>
                    <p className={`text-xs font-mono font-bold mt-1 ${selectedLog.status === "Blocked" ? "text-red-400" : "text-slate-400"}`}>{selectedLog.violatedRule}</p>
                  </div>
                  <div>
                    <span className="text-xs text-slate-500 uppercase font-semibold">User Dialog Payload</span>
                    <div className="p-3 bg-background border border-panel-border rounded-lg text-xs font-mono text-slate-300 mt-1 whitespace-pre-wrap select-all leading-relaxed max-h-40 overflow-y-auto">{selectedLog.query}</div>
                  </div>
                  <div>
                    <span className="text-xs text-slate-500 uppercase font-semibold">Gateway Action/Response</span>
                    <div className="p-3 bg-background border border-panel-border rounded-lg text-xs font-mono text-slate-300 mt-1 whitespace-pre-wrap leading-relaxed max-h-40 overflow-y-auto">{selectedLog.response}</div>
                  </div>
                </div>
              </div>
              <div className="border-t border-panel-border pt-4 mt-4">
                <button onClick={() => setSelectedLog(null)} className="w-full py-2 bg-slate-800 hover:bg-slate-700 text-xs font-bold rounded-lg transition-colors text-slate-300 uppercase tracking-wider">
                  Close Inspector
                </button>
              </div>
            </div>
